...
geometric_object_list get_GDSII_prisms(material_type material, const char *GDSIIFile, int Layer, double centerX, double centerY,
                                       double zmin, double zmax) {
  geometric_object_list prisms = {0, 0};
  ...
  for (int np = 0; np < num_prisms; np++) {
    ...
    std::unique_ptr<vector3[]> vertices(new vector3[num_vertices]);
    for (int nv = 0; nv < num_vertices; nv++) {
      vertices[nv].x = polygon[2 * nv + 0] - centerX;
      vertices[nv].y = polygon[2 * nv + 1] - centerY;
      vertices[nv].z = zmin;
    }
    double height = zmax - zmin;
    vector3 zaxis = {0, 0, 1};
