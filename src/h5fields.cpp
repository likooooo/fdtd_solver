...
#define UNUSED(x) (void)x // silence compiler warnings
                                    ...
                                    void *data_)
    {
        ...
        for (int i = 0; i < data->rank; ++i)
        {
            ...
            if (ied < isd)
                offset[permute.in_direction(d)] = count[i] - 1;
        }
        for (int i = 0; i < data->rank; ++i)
        {
            direction d = data->ds[i];
                             ...
                             double frequency)
    {
        ...
        {
            if (rank >= 3)
                meep::abort("too many dimensions in output_hdf5");
            size_t n =
        ...
        if (needs_dielectric)
            ...
            {
                if (data.ninveps == 3)
                    meep::abort("more than 3 field components??");
